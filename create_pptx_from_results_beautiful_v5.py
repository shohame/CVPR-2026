...
output_pptx = results_dir / "Recovered_Audio_Results_beautiful_v5.pptx"
...
def add_top_header(slide, setup_name):
    """Add a white top header with one combined black title."""
    add_shape(slide, MSO_SHAPE.RECTANGLE, 0, 0, SLIDE_W, 0.82, COL_WHITE)
    add_shape(slide, MSO_SHAPE.RECTANGLE, 0, 0.82, SLIDE_W, 0.02, COL_LINE)

    add_textbox(
        slide,
        f"Recovered Audio Results - {setup_name}",
        0.45,
        0.15,
        12.1,
        0.45,
...
def add_title_slide(prs):
    ...
    set_slide_background(slide, COL_WHITE)
...
def add_method_panel(slide, method, idx, total):
    """Add a compact panel with spectrogram and audio control."""
    margin_x = 0.28
    gap = 0.12
    panel_top = 4.42
    panel_h = 2.95
...
def build_presentation():
    ...
    for row_idx, row in enumerate(data_rows):
        ...
        add_shape(slide, MSO_SHAPE.ROUNDED_RECTANGLE, 3.6, 0.98, 5.8, 2.62, COL_WHITE, COL_LINE, 1)
        try:
            add_picture_contain(slide, setup_img_path, 3.85, 1.15, 5.3, 2.22)
